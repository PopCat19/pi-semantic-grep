...
function resolveEnvVars(config: SemanticGrepConfig): SemanticGrepConfig {
	const apiKey = config.embeddings.apiKey;
	if (apiKey && /^\$[A-Z_][A-Z0-9_]*$/.test(apiKey)) {
		const envVal = process.env[apiKey.slice(1)];
		if (envVal) {
			config.embeddings.apiKey = envVal;
		}
	}
	return config;
}
